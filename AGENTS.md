...
### 文档与记忆维护
- `.workbuddy/` 已退役；不要重新创建或写入 WorkBuddy memory 目录。
- 项目级长期规则写入 `AGENTS.md`；踩坑和反复出现的修复经验写入 `PITFALLS.md`。
- 架构、数据源、采集流程、发布验证等内容优先维护现有可追踪 Markdown 文档，例如 `ARCHITECTURE.md`、`DATA_ACQUISITION.md`、`DATA_SOURCES.md`、`OPEN_SOURCE_*`。
- 收储扫描结果继续以机器可读 JSON 写入 `skills/storage-event-scanner/results/weekly/YYYY-MM-DD/`，不要另建隐藏记忆目录。
- 需要浏览器交互时，优先使用 Codex 内置/应用内浏览器；不要默认启动外部 Chrome 或 `browser-use --headed`，除非用户明确要求。
- 如果需要新的文档结构，按当前任务建立最小、可追踪、用途明确的 Markdown 文件或目录，不为“记忆”单独建立隐藏 worktree。
